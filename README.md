...
docs/schema.dbml                # dbdiagram.io (DBML) source for the ERD
docker/entrypoint.sh            # migrate â†’ seed â†’ start
Dockerfile
docker-compose.yml
```
...
DB Diagram ERD: <https://dbdiagram.io/d/Rekadigi-Backend-Assessmen-6ab5240b0f25a52d01f91b20>

The same schema is kept in-repo as a dbdiagram.io (DBML) source file at
[`docs/schema.dbml`](docs/schema.dbml), so the diagram can be regenerated by
importing that file into <https://dbdiagram.io>. It mirrors the SQL migrations
exactly: every table, column, index name, check constraint and foreign-key
delete rule. The migrations remain the authoritative source; the DBML is a
representation. Category tree strategy and indexing decisions are explained in
...
**Live deployment:** <https://rekadigi-assesmen.vercel.app>

The API is deployed on Vercel (Bun runtime) against a managed PostgreSQL
database, and is testable without any local setup:

| Endpoint | URL |
|---|---|
| API base | <https://rekadigi-assesmen.vercel.app> |
| Readiness (includes DB check) | <https://rekadigi-assesmen.vercel.app/health> |
| Liveness | <https://rekadigi-assesmen.vercel.app/health/live> |
| OpenAPI UI (Swagger) | <https://rekadigi-assesmen.vercel.app/docs> |
| OpenAPI JSON | <https://rekadigi-assesmen.vercel.app/docs/json> |

No authentication is required to evaluate any endpoint. `vercel.json` pins
`bunVersion` and the `elysia` framework preset so Vercel uses `src/app.ts` (the
default-exported Elysia handler) rather than running `src/dev-server.ts`.

Deployment can also be **container-based and self-contained** via Docker Compose:
